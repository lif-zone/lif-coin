...
const hash256 = require('bcrypto/lib/hash256.js');
const hash256lif = require('../utils/hash256lif.js');
...
class Network {
  ...
  constructor(options) {
    ...
    this.pow_hash256_name = options.hash256||'hash256';
    if (this.pow_hash256_name=='hash256')
      this.pow_hash256 = hash256;
    else if (this.pow_hash256_name=='hash256lif')
      this.pow_hash256 = hash256lif;
    else
      assert();
    assert(this.pow_hash256, 'invalid hash256 type');
    this.consensus = options.consensus||networks.main.consensus;
  ...
  static set(type) {
    ...
    Network.type = type;
    for (let [k, v] of Object.entries(Network.primary.consensus))
      consensus[k] = v;
  ...
  static get_pow_hash256(network){
    if (!network && !Network.primary)
      return hash256;
    return Network.get(network).pow_hash256;
  }
}
...
Network.main = null;
Network.lifmain = null;
Network.testnet = null;
Network.liftest = null;
