...
class Chain extends AsyncEmitter {
  ...
  async isMainHash(hash) {
    return await this.db.isMainHash(hash);
  }
  ...
  async isMainChain(entry) {
    return await this.db.isMainChain(entry);
  }
  ...
  async getAncestor(entry, height) {
    return await this.db.getAncestor(entry, height);
  }
  ...
  async getPrevious(entry) {
    return await this.db.getPrevious(entry);
  }
  ...
  async getNext(entry) {
    return await this.db.getNext(entry);
  }
  ...
  async getNextEntry(entry) {
    return await this.db.getNextEntry(entry);
  }
  ...
  async getEntry(hash) {
    return await this.db.getEntry(hash);
  }
  ...
  async getEntryByHeight(height) {
    return await this.db.getEntryByHeight(height);
  }
  ...
  async getEntryByHash(hash) {
    return await this.db.getEntryByHash(hash);
  }
  ...
  async getHash(height) {
    return await this.db.getHash(height);
  }
  ...
  async getHeight(hash) {
    return await this.db.getHeight(hash);
  }
  ...
  async hasEntry(hash) {
    return await this.db.hasEntry(hash);
  }
  ...
  async getNextHash(hash) {
    return await this.db.getNextHash(hash);
  }
  ...
  async hasCoins(tx) {
    return await this.db.hasCoins(tx);
  }
  ...
  async getHashes(start = -1, end = -1) {
    return await this.db.getHashes(start, end);
  }
  ...
  async readCoin(prevout) {
    return await this.db.readCoin(prevout);
  }
  ...
  async getCoin(hash, index) {
    return await this.db.getCoin(hash, index);
  }
  ...
  async getRawBlock(block) {
    return await this.db.getRawBlock(block);
  }
  ...
  async getBlockView(block) {
    return await this.db.getBlockView(block);
  }
  ...
  async getCoinView(tx) {
    return await this.db.getCoinView(tx);
  }
  ...
  async getTarget(time, prev) {
    ...
    if ((prev.height + 1) % pow.retargetInterval !== 0) {
      ...
      // BUGFIX: original code did not have this check, so if initial genesis
      // block had a target below min (pow.limit) - this minimum difficulty
      // rule was not enforced.
      if (consensus.fromCompact(prev.bits).gt(pow.limit))
        return pow.bits;
      return prev.bits;
    }
