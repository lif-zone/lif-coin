...
const lif_kv_idx = require('./lif_kv_idx');
...
class Mempool extends EventEmitter {
  ...
  constructor(options){
    ...
    this.addrindex = new AddrIndexer(this.network, options);
    if (this.options.lif_kv_idx)
      this.lif_kv_idx = new lif_kv_idx(this.network, options);
  }

  dag_sort(dag){
   // count how many unresolved deps each node has
   const depCount = {};
   // for each dep, which nodes are waiting on it
   const waiters = {};
   ...
   while (queue.length){
     ...
     result.push(node);
     for (const waiter of (waiters[node]||[])){
       depCount[waiter]--;
       if (!depCount[waiter])
         queue.push(waiter);
     }
   }
   assert(result.length==Object.keys(depCount).length, 'cycle detected');
   return result;
 }
  // make sure that a tx that is used as inputs in other tx - appears before
  // the spender tx
  entries_dag_sort(entries){
    let dag = {};
    let all = {};
    ...
    for (const e of entries){
      ...
      for (let i=0; i<tx.inputs.length; i++){
        let _in = tx.inputs[i];
        let h = util.revHex(_in.prevout.hash);
        if (!all[h])
          continue;
        child.push(h);
      }
  ...
  async open() {
    ...
    if (this.options.persistent){
      ...
      entries = this.entries_dag_sort(entries);
      for (const entry of entries){
        await this.trackEntry(entry);
        this.updateAncestors(entry, addFee);
        if (this.options.indexAddress){
          // also_spent because we just now already included (tracked) this tx
          const view = await this.getCoinView(entry.tx, true);
          this.indexEntry(entry, view);
        }
        if (this.lif_kv_idx)
          this.lif_kv_add(entry.tx);
      }
      this.logger.info(
  ...
  async _addBlock(block, txs) {
    ...
    for (let i = txs.length - 1; i >= 1; i--) {
      ...
      if (!entry) {
        ...
        await this.removeDoubleSpends(tx);
  ...
  getMetaByAddress(addr, options) {
    return this.getMetaByAddrSH(addr, options); // bcoin compat
  }
  getMetaByAddrSH(addr, options){
  ...
  async insertTX(tx, id) {
    ...
    if (this.exists(hash)) {
      this.logger.info('txn-already-in-mempool');
      return;
      throw new VerifyError(tx,
        'alreadyknown',
    ...
    // mempool.
    // XXX TODO: a more sophisticated implementation can allow parallel options
    // for mutually exclusive transactions - so when building a block, either
    // of the TXs can be chosen (but not both) - according to whoever pays
    // higher fees.
  ...
  async addEntry(entry, view) {
    ...
    await this.trackEntry(entry, view);
  ...
  async evictEntry(entry) {
    await this.removeSpenders(entry);
  ...
  async removeSpenders(entry) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      await this.removeSpenders(spender);
      await this.removeEntry(spender);
    }
  }
  ...
  async getCoinView(tx, also_spent) {
    const view = new CoinView();
    ...
    for (const {prevout} of tx.inputs) {
      ...
      if (tx) {
        if (this.hasCoin(hash, index) || also_spent)
          view.addIndex(tx, index, -1);
        continue;
      }
  ...
  async trackEntry(entry, view) {
    ...
    if (this.options.indexAddress && view)
      this.indexEntry(entry, view);
    if (this.lif_kv_idx)
      await this.lif_kv_add(entry.tx);
  ...
  lif_kv_add(tx){
    this.lif_kv_idx.add(tx/*, kv_this.chain*/);
  }
  lif_kv_rm(tx){
    this.lif_kv_idx.rm(tx);
  }
...
class MempoolOptions {
  ...
  fromOptions(options){
    ...
    if (options.lif_kv_idx)
      this.lif_kv_idx = options.lif_kv_idx;
    return this;
  }
