 ...
 *  a[addrsh][height][index] -> dummy (tx by scripthash)
 *  -> a[addrsh][height][index][vout] -> spent height+index+vin (tx by scripthash)
...
Object.assign(layout, {
  ...
  a: bdb.key('a', ['hash256', 'uint32', 'uint32']),
...
class AddrIndexer extends Indexer {
  ...
  constructor(options){
    ...
    this.indexAddrSH = options.indexAddrSH;
  }
  ...
  async indexBlock(meta, block, view){
    ...
    for (let i = 0; i < block.txs.length; i++){
      ...
      if (this.indexAddrSH){
        for (const addrsh of tx.getAddressesSH(view)){
          this.put(layout.a.encode(addrsh, height, i), null);
          hasAddress = true;
        }
      }
      if (hasAddress){
  ...
  async unindexBlock(meta, block, view){
    ...
    for (let i = block.txs.length-1; i>=0; i--){
      ...
      if (this.indexAddrSH){
        for (const addrsh of tx.getAddressesSH(view)){
          this.del(layout.a.encode(addrsh, height, i));
          hasAddress = true;
        }
      }
      if (hasAddress){
  ...
  async getHashesByAddrSH(addr, options = {}){
    if (!addr.addrsh)
      return await this.getHashesByAddress(addr, options);
    const {after, reverse} = options;
    let {limit} = options;
    if (!limit)
      limit = this.maxTxs;
    const hash = addr.addrsh;
    const opts = {
      limit,
      reverse,
      parse: key=>{
    ...
    if (after && hasAfter){
      ...
      const {height, index} = count;
      if (!reverse){
        opts.gt = layout.a.min(hash, height, index);
        opts.lte = layout.a.max(hash);
      } else {
        opts.gte = layout.a.min(hash);
        opts.lt = layout.a.max(hash, height, index);
      }
    } else {
      // Give earliest or latest results
      // for the address.
