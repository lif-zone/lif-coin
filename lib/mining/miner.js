...
class Miner extends EventEmitter {
  ...
  async createBlock(tip, addresses) {
    const unlock = await this.locker.lock();
    try {
      return await this._createBlock(tip, addresses);
    } finally {
      unlock();
    }
  }
  ...
  async _createBlock(tip, addresses) {
...
class MinerOptions {
  ...
  constructor(options) {
    ...
    this.version = -1;
    this.addresses = [];
    this.coinbaseFlags = Buffer.from('mined by bcoin', 'utf8');
