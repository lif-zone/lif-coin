...
class BlockTemplate {
  ...
  fromOptions(options) {
    ...
    if (options.addresses != null)
      this._setAddresses(options.addresses);
  ...
  getReward() {
    const reward = consensus.getReward(this.height,
      this.network.halvingInterval);
    return reward + this.fees;
  }

  /**
   * Initialize the default coinbase.
   * @param {Buffer} segwit_hash - Witness commitment hash.
   * @returns {TX}
   */

  createCoinbase(segwit_hash) {
    ...
    // for Mining pools to provide to each miner a different template
    ...
    assert(this.addresses.length, 'mine: no addresses defined');
    // Reward outputs.
    let value_sum = this.addresses.reduce((sum, a)=>sum+(a.value||0), 0);
    let reward = this.getReward();
    if (value_sum>reward)
      throw Error('mine: value '+value_sum+'more than reward '+reward);
    let value_rest = reward-value_sum;
    for (const a of this.addresses){
      const output = new Output();
      output.script.fromAddress(a.address);
      output.value = a.value==null ? value_rest : a.value;
    ...
    if (this.witness) {
      ...
      commit.script.fromCommitment(segwit_hash);
    ...
    } else if (0) { // skip these test, due to dynamic output addresses
  ...
  refresh() {
    const segwit_hash = this.getWitnessHash();
    const cb = this.createCoinbase(segwit_hash);
  ...
  getProof(nonce1, nonce2, time, nonce) {
    ...
    const hash = Network.get_pow_hash256().digest(data);
  ...
  toBlock(nonce1=0, nonce2=0, time=this.time, nonce=0){
    const proof = this.getProof(nonce1, nonce2, time, nonce);
  ...
  _setAddresses(addresses){
    if (typeof addresses=='string' || addresses instanceof Address)
      this.addresses = [{address: new Address(addresses)}];
    else if (!addresses?.length)
      throw Error('missing coinbase address');
    else if (Array.isArray(addresses)){
      let rest = 0;
      this.addresses = [];
      for (let a of addresses){
        if (typeof a=='string' || a instanceof Address)
          a = {address: a};
        let addr = new Address(a.address);
        if (a.value==null){
          if (rest)
            throw Error('only one coinbase address can be rest');
          rest++;
        }
        this.addresses.push({...a, address: addr});
      }
      if (rest!=1)
        throw Error('one mine address must be rest');
    } else
      throw Error('invalid coinbase address');
  }
