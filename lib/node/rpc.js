...
class RPC extends RPCBase {
  ...
  // derry: nice indentation! 80% good (no {} on single line if()
  ...
  init() {
    ...
    this.add('getblockstats', this.getBlockStats);
    ...
    this.add('getindexinfo', this.getIndexInfo);
  ...
  // added function just to be able to log calls
  async call(body, query){
    let res = await super.call(body, query);
    0 && console.log(body,query, res);
    return res;
  }
  /*
  ...
  async getBlockStats(args, help) {
    ...
    if (height==-1)
      hash = valid.brhash(0);
    let stats = valid.array(1);
    if (height!=-1){
      if (height>this.chain.height)
        throw new RPCError(errs.INVALID_PARAMETER, 'Block height out of range.');
      hash = await this.chain.getHash(height);
      if (!hash)
        throw new RPCError(errs.MISC_ERROR, 'Not found.');
    }
    let entry = this.chain.getEntry(hash);
    if (!entry)
      throw new RPCError(errs.MISC_ERROR, 'Block hash not found.');
    // XXX TODO: put real values. This is bitcoin block #900000
    let exp = {
      avgfee: 14823, // Average transaction fee in the block (in satoshis). 
                     // Calculated as totalfee / (txs - 1) — excludes coinbase tx which has no fee.
                     // Implement: sum fees of all non-coinbase txs, divide by non-coinbase tx count.
      avgfeerate: 38, // Average feerate across all non-coinbase transactions (satoshis per virtual byte, vB).
                      // Uses SegWit discounted weight: feerate = fee / (weight / 4).
                      // Implement: weighted average of each tx's feerate.
      avgtxsize: 548, // Average serialized size of non-coinbase transactions (in bytes).
                      // Implement: total_size / (txs - 1).
      blockhash: hash, // "00000000000000000002a7c4c1e48d4f0b9e7f7c4a2b3d8e5f6a1b2c3d4e5f6a", 
                 // The full 64-character hex block hash.
                 // Always include this — useful for clients to detect reorgs.
      feerate_percentiles: [12, 22, 38, 65, 110], 
                 // Array of 5 feerates (sat/vB) at the 10th, 25th, 50th (median), 75th, and 90th percentiles.
                 // Sorted list of all non-coinbase tx feerates, then pick percentiles by weight.
                 // Implement carefully: sort txs by feerate, use cumulative weight to find thresholds.
      height: entry.height,
                 // 900000, // The block height (integer). Always matches the requested height if block exists.
      ins: 4123, // Total number of inputs across all non-coinbase transactions.
                 // Counts spent UTXOs (excluding coinbase which has no real inputs).
      maxfee: 3125000, // Highest fee paid by any single transaction in the block (satoshis).
      maxfeerate: 152, // Highest feerate of any transaction (sat/vB).
      maxtxsize: 142000, // Largest transaction size in the block (bytes). Usually close to legacy limit or SegWit max.
      medianfee: 11234, // Median fee (50th percentile fee, truncated — not weighted).
      mediantime: 1726248000, // Median timestamp of the block (Unix epoch seconds). From block header's nTime of recent blocks.
      mediantxsize: 512, // Median transaction size (bytes, truncated).
      minfee: 500, // Lowest fee paid by any non-zero-fee transaction (satoshis). May be 0 if dust/zero-fee txs.
      minfeerate: 1, // Lowest feerate (sat/vB). Often 1 during low-fee periods.
      mintxsize: 192, // Smallest transaction size (bytes). Usually a simple P2WPKH spend.
      outs: 6789, // Total number of outputs created across all transactions (including coinbase).
      subsidy: 312500000, // Block reward/subsidy in satoshis (3.125 BTC = 312,500,000 sat after 2024 halving).
      swtotal_size: 1123456, // Total serialized size of all SegWit transactions (bytes, witness stripped? Wait — actually total vsize contribution).
      swtotal_weight: 3890123, // Total weight units of SegWit transactions (WU). SegWit txs contribute to weight.
      swtxs: 2341, // Number of transactions that use SegWit (have witness data).
      time: entry.time, // 1726249200, // Block header timestamp (Unix epoch seconds).
      total_out: 9876543210123, // Total value of all outputs excluding the coinbase reward (satoshis).
                                // Sum of all output amounts minus subsidy.
      total_size: 1324567, // Total serialized size of non-coinbase transactions (bytes, stripped if SegWit).
      total_weight: 3992345, // Total weight units of non-coinbase transactions (max ~4,000,000 WU per block).
      totalfee: 45678901, // Sum of all fees collected in the block (satoshis). Miner revenue besides subsidy.
      txs: 3082, // Total number of transactions in the block, including the coinbase (always at least 1).
      utxo_increase: 2666, // Net change in UTXO set size from this block (outs - ins).
      utxo_size_inc: 212345, // Net change in serialized UTXO set size (bytes added/removed).
                             // Useful for estimating database growth.
    };
    return exp;
  }
  ...
  async getIndexInfo(args, help){
    if (help || args.length > 1)
      throw new RPCError(errs.MISC_ERROR, 'getblockfilter ( "txinfo" )');
    const valid = new Validator(args);
    const idx = valid.str(1, '');
    let res = {txindex: {
      synced: true, // !!this.chain.synced,
      best_block_height: this.chain.tip.height,
    }};
    return idx ? res[idx] : res;
  }
    ...
    for (const deploy of this.network.deploys) {
      ...
      switch (state) {
        ...
        case common.thresholdStates.LOCKED_IN:
          ...
          // eslint-enable-line no-fallthrough
  ...
  async getRawTransaction(args, help) {
    ...
    const hash = valid.brhash(0);
    const verbose = valid.bool(1, false);
    const blockhash = valid.brhash(2); // TODO: currently ignored
    // blockhash (string, optional): The hash of the specific block to search for the transaction in.
    // This allows retrieving the transaction even without -txindex=1, as long as your node has that exact block available (not pruned).
    // Useful for historical transactions on pruned nodes or to avoid full txindex overhead.
    // If provided, bitcoind looks only in that block (ignores mempool and other blocks).
    // If the transaction isn't in that block, it returns an error (e.g., "No such transaction found").
  ...
  async estimateSmartFee(args, help) {
    ...
    let mode = valid.str(1, ''); // 'CONSERVATIVE'
    if (!this.fees)
      throw new RPCError(errs.MISC_ERROR, 'Fee estimation not available.');
    if (blocks>this.fees.feeStats.maxConfirms)
      blocks = this.fees.feeStats.maxConfirms;
