...
const mining_common = require('../mining/common');
...
class HTTP extends Server {
  ...
  init(){
    ...
    this.init_bcoin_wallet(); // bcoin wallet over http
    this.init_btc_rpc(); // btc jsonrpc over websocket
    this.init_lifcoin_http(); // lifcoin over http
    if (this.mempool)
      this.mine_init();
    this.init_websocket_rpc();
  }
  init_websocket_rpc(){
    // a hack to catch bweb/bsock upgrade, and route requests by url
    let attach = this.io.attach.bind(this.io);
    let on_bcoin_rpc;
    attach({on: (upgrade, fn)=>{
      assert(upgrade=='upgrade');
      on_bcoin_rpc = fn;
    }});
    assert(on_bcoin_rpc!=null, 'failed attach');
    this.io.attach = server=>{
      server.on('upgrade', (req, socket, head)=>{
        let p = URL.parse(req.url, 'http://x/').pathname;
        if (p=='/socket.io/')
          return on_bcoin_rpc(req, socket, head);
        if (p=='/electrum')
          return this.on_electrum(req, socket, head); // electrum jsonrpc over websocket
        socket.emit('close');
        server.emit('error', 'not found: '+p);
        socket.destroy();
      });
    };
  }
  ...
  init_lifcoin_http(){
    ...
    this.get('/blockstream/block-template', async(req, res)=>{
      let ret = await this.rpc.getBlockTemplate([{rules: ['segwit']}]);
      res.json(200, ret);
    });
    ...
    // LIF KV: called from lif-kernel/hi.js
    this.get('/lif_kv', async(req, res)=>{
      const valid = Validator.fromRequest(req);
      const key = valid.str('key');
      enforce(key, 'missing key');
   ...
   */
  handleSocket(socket){
    this.handle_bcoin_wallet_rpc(socket);
  }
  handle_bcoin_wallet_rpc(socket){
    socket.hook('auth', (...args)=>{
      ...
      this.handle_bcoin_wallet_auth(socket);
      return null;
    });
    socket.fire('version', {
      version: pkg.version,
   ...
   * @param {WebSocketIO} socket
   */
  handle_bcoin_wallet_auth(socket){
  ...
  on_electrum(req, socket, head){
    // https://electrumx.readthedocs.io/en/latest/protocol-methods.html
    //const wss = new WebSocketServer({server: this.http, path: '/electrum'});
    let ws_server = new WebSocketServer({noServer: true});
    let electrum = new WebSocketServer_json_rpc({ws_server});
    ws_server.handleUpgrade(req, socket, head,
      ws=>ws_server.emit('connection', ws));
    electrum.method('server.version', ()=>['lifcoin', '1.4']);
    electrum.method('server.banner', ()=>'lifcoin');
    electrum.method('server.ping', ()=>null);
    electrum.method('server.features', ()=>{
      ...
      let res = {
        ...
        hash_function_pow: network.pow_hash256_name,
        network: network.type,
      };
      return res;
    });
    electrum.method('server.donation_address',
      ()=>this.miner.getAddress().toString(this.network));
    function electrum_addrsh(sh){
      // electrum reverses the scripthash
      const b = Buffer.from(sh, 'hex').reverse();
      if (b.length!=32)
        throw Error('invalid scripthash '+sh);
      return b;
    }
    electrum.method('blockchain.scripthash.get_balance', async([sh])=>{
    ...
    electrum.method('blockchain.scripthash.get_history', async([sh])=>{
      let history = [];
      let addrsh = electrum_addrsh(sh);
      let metas = await this.node.getMetaByAddrSH({addrsh});
      for (const meta of metas){
    ...
    electrum.method('blockchain.scripthash.listunspent', async([sh])=>{
      let unspent = [];
      let addrsh = electrum_addrsh(sh);
      let metas = await this.node.getMetaByAddrSH({addrsh});
      for (const meta of metas){
    ...
    electrum.method('blockchain.block.header', async([height])=>{
      let block = await this.chain.getEntryByHeight(height);
      if (!block)
        return {error: 'not found', code: -5};
      return block.toRaw().toString('hex');
    });
    electrum.method('blockchain.transaction.get', async([tx_hash, verbose])=>{
    ...
    electrum.method('blockchain.transaction.broadcast', async([raw_tx])=>{
      const tx = TX.fromRaw(Buffer.from(raw_tx, 'hex'));
      await this.node.sendTX(tx);
      return tx.rhash();
    });
    electrum.method('blockchain.lif_kv.get', async([key])=>{
      let kv = await this.node.lif_kv_get(key);
      if (!kv)
        return {error: 'not found', code: -5};
      return kv;
    });
    electrum.method('blockchain.estimatefee', async([nblocks])=>{
      return this.network.feeRate/100_000_000;
    });
    electrum.method('blockchain.mine.get_template', async([addr])=>{
      let addr_self = this.miner.addresses[0];
      let offer = await this.mine_attempt_get({addr, addr_self});
      let ret = {header: offer.header.toString('hex'),
        reward_share: offer.reward_share,
        reward: offer.reward,
        block_reward: offer.block_reward,
        coinbase_raw: offer.coinbase_raw.toString('hex')};
      if (0){
        const header = offer.header;
        const target = mining_common.getTarget(header.readUInt32LE(72));
        const found_n0 = mine(header, target, 0, 0xffffffff); // 0.28M/sec
        console.log('found n0', found_n0);
        header.writeUInt32LE(found_n0, 76);
        const hash = this.network.pow_hash256.digest(header);
        console.log('hash', hash.toString('hex'));
      }
      return ret;
    });
    electrum.method('blockchain.mine.submit_header', async([header])=>{
      const _header = Buffer.from(header, 'hex');
      let res = await this.mine_submit_header(_header);
      if (res.err)
        return {err: res.err};
      return {ok: 'block added', height: res.entry.height};
    });
  }

  // assemble a new attempt block no more than once in 5 seconds
  mine_assemble_refresh = 5;
  mine_init(){
    this.mempool.on('add entry', ()=>{
      if (!this.mine_offers)
        return;
      this.mine_offers.mempool_seq++;
    });
  }
  async mine_attempt_get({addr, addr_self, nonce1=0, nonce2=0}){
    assert(addr, 'mine: missing addr');
    assert(addr_self, 'mine: missing addr_self (coinbase-address)');
    let mo = this.mine_offers;
    if (!mo || mo.tip!==this.chain.tip){
      mo = this.mine_offers = {
        offer: {},
        time: null,
        tip: this.chain.tip,
        mempool_seq: 0,
        mempool_seq_last: null,
      };
    }
    let time = this.network.now();
    if (!mo.time ||
      (mo.mempool_seq!=mo.mempool_seq_last &&
      mo.time<=time-this.mine_assemble_refresh))
    {
      // assemble new block from mempool
      mo.mempool_seq_last = mo.mempool_seq;
      mo.time = time;
      mo.attempt_wait = this.miner.createBlock(null, this.miner.addresses);
    }
    if (!mo.attempt)
      mo.attempt = await mo.attempt_wait;
    const attempt = mo.attempt;
    const saddr = addr?.toString(this.network) || '';
    const saddr_self = addr_self?.toString(this.network) || '';
    const reward_share = 0.9; // 90% reward to miner, 10% to this node (self)
    const block_reward = consensus.getReward(
      attempt.height, this.network.halvingInterval);
    const reward = Math.floor(reward_share*block_reward);
    const addrs = [{address: addr, value: reward}, addr_self];
    attempt.setAddresses(addrs);
    const coinbase_raw = attempt.getRawCoinbase(nonce1, nonce2);
    const header = attempt.getHeader(
      attempt.getRoot(nonce1, nonce2), time, 0);
    let offer = {
      nonce1,
      nonce2,
      time,
      time_assemble: mo.time,
      attempt,
      saddr,
      saddr_self,
      reward_share,
    ...
    offer.key = this.mine_header_key(offer.header),
    mo.offer[offer.key] = offer;
    return offer;
  }
  mine_header_key(header){
    let h = Buffer.from(header);
    h.writeUInt32LE(0, 68); // clear time
    h.writeUInt32LE(0, 76); // clear nonce0
    return h.toString('hex');
  }
  async mine_submit_header(header){
    const hash = this.network.pow_hash256.digest(header);
    const target_a = mining_common.getTarget(header.readUInt32LE(72));
    const time = header.readUInt32LE(68);
    const nonce0 = header.readUInt32LE(76);
    const found = mining_common.rcmp(hash, target_a)<=0;
    const _header = header.toString('hex');
    if (!found)
      return {err: 'not a winning target header'};
    const key = this.mine_header_key(header);
    const mo = this.mine_offers;
    if (!mo)
      return {err: 'winning target without offer'};
    const offer = mo.offer[key];
    if (!offer)
      return {err: 'winning target wihtout offer found'};
    // add block offer.attempt;
    const block = offer.attempt.toBlock(offer.nonce1, offer.nonce2,
      time, nonce0);
    let entry = await this.chain.add(block);
    return {entry};
  }
...
class WebSocketServer_json_rpc extends EventEmitter {
  ...
  init_conn(ws){
    ...
    ws.on('message', async(message)=>{
      ...
      let fn;
      if (!('jsonrpc' in req))
        return send({id, error: 'not jsonrpc'});
      if (typeof req.jsonrpc!='string')
        return send({id, error: 'invalid jsonrpc version'});
      if (!('params' in req))
        req.params = [];
      if (!('method' in req))
        return send({id, error: 'params missing'});
      if (!(fn=this.methods[req.method]))
        return send({id, error: 'method missing: '+req.method});
      let ret;
      try {
