...
elif [ "$1" == is-enable ] ; then
  sudo systemctl is-enable lif-kernel
  sudo systemctl is-enable lif-coin
  sudo systemctl is-enable lif-coin-lifnet
else
  echo "lif_service.sh start|stop|restart|status|enable|disable|is-enable"
fi
